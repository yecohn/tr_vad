...
class FeedForward(nn.Module):
    def __init__(
        self, units=16, dim=128, P=16, ratio=4, drop_rate=0.3, activation=nn.ReLU()
    ):
        super().__init__()
    ...
    def forward(self, x):
        ...
        x = rearrange(x, "b (c p1) (d p2)-> b (c d) p1 p2", p1=self.P, p2=self.P)
        x = self.net(x)
...
class MultiHeaded(nn.Module):
    def __init__(
        self, h=8, units=16, d_model=392, drop_rate=0.3, P=16, activation=nn.ReLU()
    ):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        x, self.attn = attention(
            query, key, value, mask=mask, dropout=self.dropout, position=self.position
        )
...
class Postnet(nn.Module):
    def __init__(
        self, dim_in=56, ratio=2, units=16, P=16, drop_rate=0.3, activation=nn.ReLU()
    ):
        super().__init__()
