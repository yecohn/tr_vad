...
def data_transform_targets_bdnn(inputs, w=8, u=2, DEVICE=DEVICE):
    ...
    trans_inputs = torch.vstack(
        [
            torch.unsqueeze(
                torch.roll(pad_inputs, int(-1 * neighbors[i]), dims=0)[
                    0 : inputs.shape[0]
                ],
                dim=0,
            )
...
def data_transform(inputs, w=8, u=2, min_abs_value=1e-7, DEVICE=DEVICE):
    ...
    pad_size = 2 * w + inputs.shape[0]
    pad_inputs = torch.ones((pad_size, inputs.shape[1])).to(DEVICE) * max(
        min_abs_value, 1e-7
    )
    pad_inputs[0 : inputs.shape[0], :] = inputs
...
class SpeechDataset(Dataset):
    def __init__(self, metadata_filename, metadata, hparams):
        ...
        self._mel_dir = os.path.join(os.path.dirname(metadata_filename), "afpc")
        self._metadata = metadata
        timesteps = sum([int(x[2]) for x in self._metadata])
        sr = hparams.sample_rate
        hours = timesteps / sr / 3600
        print(
            "Loaded metadata for {} examples ({:.2f} hours)".format(
...
class MSDWildDataset(Dataset):
    def __init__(self, pickle_path, hparams):
        with open(pickle_path, "rb") as f:
            self.dataset = pickle.load(f)
        self._hparams = hparams
        # self.dataset = self.rebatch()

    def __len__(self):
        return len(self.dataset)

    # def rebatch(self):
    #     feature_input_v = None
    #     target_v = None
    #     for sample_id in tqdm(range(len(self.dataset))):
    #         # for sample_id in tqdm(range(5)): #debug batch
    #         sample = self.dataset[sample_id]
    #         feature_input = sample["input"][:, :80]
    ...
    #             target, self._hparams.w, self._hparams.u, DEVICE=torch.device("cpu")
    #         )
    #         if feature_input_v is None:
    #             feature_input_v = feature_input
    #         feature_input_v = np.concatenate([feature_input_v, feature_input], axis=0)
    #         if target_v is None:
    #             target_v = target
    #         target_v = np.concatenate([target_v, target], axis=0)
    #     dic = dict(input=feature_input_v, target=target_v)
    #     return dic
...
def train_valid_split(metadata_filename, test_size=0.05, seed=0):
    ...
    aug_list = [
        [idx, x] for idx, x in enumerate(data) if len(Path(x[0]).name.split("_")) > 3
    ]
    snr_list = list(set([Path(x[0]).name.split("_")[3] for idx, x in aug_list]))
    noise_list = list(set([Path(x[0]).name.split("_")[4] for idx, x in aug_list]))
    ...
    for n in noise_list:
        ...
        for s in snr_list:
            meta_idx[n][s] = [
                idx
                for idx, x in aug_list
                if Path(x[0]).name.split("_")[4] == n
                and Path(x[0]).name.split("_")[3] == s
            ]
...
def get_parameter_number(net):
    ...
    print(
        "Total: {:.3f}Million, Trainable: {:.3f}Million".format(
